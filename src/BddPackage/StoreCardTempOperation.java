...
public class StoreCardTempOperation extends BDD<StoreCard> {

    @Override
    public boolean insert(StoreCard o) {
        connectDatabase();
        boolean ins = false;
        String query = "INSERT INTO STORE_CARD_TEMP (ID_INPUT, ID_ARTICLE, DATE_STORE , QTE_STORED , QTE_CONSUMED, PRICE ) VALUES (?,?,?,?,?,?) ; ";
        try {
    ...
    public boolean update(StoreCard o1, StoreCard o2) {
        connectDatabase();
        boolean upd = false;
        String query = "UPDATE STORE_CARD_TEMP SET DATE_STORE = ?, PRICE = ? , QTE_STORED = ? , QTE_CONSUMED = ? WHERE ID = ?";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public boolean updateDateStoreByInput(LocalDate date, int idInput) {
        connectDatabase();
        boolean upd = false;
        String query = "UPDATE STORE_CARD_TEMP SET DATE_STORE = ? WHERE ID_INPUT = ?";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
            preparedStmt.setDate(1,Date.valueOf(date));
            preparedStmt.setInt(2,idInput);
    ...
    public boolean delete(StoreCard o) {
        connectDatabase();
        boolean del = false;
        String query = "DELETE FROM STORE_CARD_TEMP WHERE ID = ? ;";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public boolean deleteByInput(int idInput) {
        connectDatabase();
        boolean del = false;
        String query = "DELETE FROM STORE_CARD_TEMP WHERE ID_INPUT = ? ;";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public boolean updateQte(StoreCard o) {
        connectDatabase();
        boolean upd = false;
        String query = "UPDATE STORE_CARD_TEMP SET QTE_CONSUMED = QTE_CONSUMED + ? WHERE ID = ? ;";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public ArrayList<StoreCard> getAll() {
        ...
        String query = "SELECT * FROM STORE_CARD_TEMP ";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public StoreCard get(int id) {
        connectDatabase();
        StoreCard storeCard = new StoreCard();
        String query = "SELECT * FROM STORE_CARD_TEMP  WHERE ID = ? ; ";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
    ...
    public ArrayList<StoreCard> getAllByInput(int idInput) {
        connectDatabase();
        ArrayList<StoreCard> list = new ArrayList<>();
        String query = "SELECT * FROM STORE_CARD_TEMP WHERE ID_INPUT = ?";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
            preparedStmt.setInt(1,idInput);
            ResultSet resultSet = preparedStmt.executeQuery();
    ...
    public ArrayList<StoreCard> getAllByArticleOrderByDate(int idArt) {
        connectDatabase();
        ArrayList<StoreCard> list = new ArrayList<>();
        String query = "SELECT * FROM STORE_CARD_TEMP WHERE ID_ARTICLE = ?  AND (QTE_STORED - QTE_CONSUMED) > 0  ORDER BY DATE_STORE DESC;";
        try {
            PreparedStatement preparedStmt = conn.prepareStatement(query);
