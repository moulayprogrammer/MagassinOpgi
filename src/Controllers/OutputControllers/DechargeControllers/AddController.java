...
import BddPackage.*;
import Controllers.OutputControllers.OutputArticlesControllers.AddDemandController;
import Controllers.OutputControllers.OutputArticlesControllers.Print;
import Controllers.OutputControllers.OutputArticlesControllers.SelectEmployeeController;
import Models.*;
...
public class AddController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        tcName.setCellValueFactory(data -> data.getValue().get(0));
        tcQte.setCellValueFactory(data -> data.getValue().get(0));
        ...
        if (dataSelected != null) {
            ...
            if ( ex == -1 ){
               try {
                   ...
                   dialog.setTitle("QUANTETIE");
                   dialog.initOwner(this.tfRecherche.getScene().getWindow());
                   dialog.setHeaderText("Entre le quantitie");
                  ...
                  /* FXMLLoader loader = new FXMLLoader(getClass().getResource("/Views/OutputViews/DechargeViews/AddDemandView.fxml"));
                   DialogPane temp = loader.load();
    ...
    private void ActionDeleteSales(){
        ...
        if (compoSelectedIndex != -1){
            int id = Integer.parseInt(dataTable.get(compoSelectedIndex).get(0).getValue());
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (Integer.parseInt(dataTable.get(i).get(0).getValue()) == id){
                    dataTable.remove(i);
                    priceList.remove(i);
                    size--;
                    i--;
    ...
    void ActionInsert(ActionEvent event) {
        ...
        try {
            ...
            if (date != null && indexEmployee != -1 && dataTable.size() != 0 ){
    ...
    private boolean insertComponentOutput(ComponentOutput componentOutput){
        ...
        try {
          //  insert = componentDechargeOperation.insert(componentOutput);
            return insert;
        }catch (Exception e){
            e.printStackTrace();
            return insert;
